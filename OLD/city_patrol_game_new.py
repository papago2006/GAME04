...
class Player:
    ...
    def draw(self, screen):
        ...
        # 1. 尺寸與顏色定義 (巨大化、外星化)
        # ------------------------------------
        self.width = 180  # 大幅加寬，充滿壓迫感
        self.height = 80  # 增加高度以容納更多結構
        
        # 顏色主題：深淵紫 + 劇毒綠 + 能量洋紅
        # 像是來自深空或未知維度的生物甲殼
        shell_color_dark = (48, 10, 60)         # 甲殼暗部 (深紫色)
        shell_color_light = (85, 40, 110)       # 甲殼亮部 (亮紫色)
        cockpit_color = (10, 0, 10)             # 完全不反光的駕駛艙
        
        # 能量顏色
        energy_glow_green = (50, 255, 150)      # 主要能量流：劇毒綠
        energy_glow_magenta = (255, 0, 255)     # 武器/核心：洋紅色
        energy_core_white = (255, 255, 255)     # 能量核心的熾白光點

        # 動態效果參數 (讓載具像活物一樣呼吸)
        # 使用 sin 函數創造 0 到 1 之間平滑變化的值
        pulse_slow = (math.sin(pygame.time.get_ticks() * 0.0015) + 1) / 2 # 慢速呼吸
        pulse_fast = (math.sin(pygame.time.get_ticks() * 0.008) + 1) / 2  # 快速脈動

        # 2. 繪製非對稱的生物力學主體
        # ------------------------------------
        # 主要結構 - 下層甲殼 (使用多邊形創造不規則的有機形狀)
        lower_shell_points = [
            (self.x, self.y + 20),
        ...
        main_crystal_points = [
            ...
            (self.x + 60, crystal_y),
            (self.x + 50, crystal_y + 15 + pulse_slow * 10) # 噴射時會變長
        ]
        pygame.draw.polygon(screen, energy_glow_green, main_crystal_points)
        pygame.draw.circle(screen, energy_core_white, (self.x + 48, crystal_y), 3 + pulse_fast * 2)

        # 前方輔助穩定水晶
        front_crystal_points = [
            (self.x + self.width - 25, crystal_y + 10),
            (self.x + self.width - 15, crystal_y - 5),
            (self.x + self.width - 5, crystal_y + 10),
            (self.x + self.width - 15, crystal_y + 20)
        ]
        pygame.draw.polygon(screen, energy_glow_green, front_crystal_points)
        
             # 4. 繪製支配級的巨型武器平台 (極致誇張)
        # ----------------------------------------------------
        # 武器基座 - 使用多邊形打造更巨大、更具威脅的形狀
        weapon_base_points = [
            (self.x + 60, self.y + 10),
            (self.x + 70, self.y - 30), # 大幅加高
        ...
        # 能量核心與發射器 - 全面放大
        core_x = self.x + 115
        core_y = self.y - 20 # 核心位置也相應提高

        # 畫一個巨大且脈動劇烈的能量光暈
        # 基礎半徑從 10 -> 22，脈動幅度從 5 -> 15
        pygame.draw.circle(screen, energy_glow_magenta, (core_x, core_y), 22 + pulse_slow * 15)
        
        # 畫中心更明亮、更不穩定的熾白核心
        # 基礎半徑從 4 -> 8，脈動幅度從 3 -> 6
        pygame.draw.circle(screen, energy_core_white, (core_x, core_y), 8 + pulse_fast * 6)
        
        # 發射器 - 從線條升級為更粗壯的能量導軌炮管 (用多邊形)
        cannon_points = [
            (core_x - 10, core_y - 8),
            (self.x + self.width + 10, core_y - 12), # 炮管伸出船體之外
            (self.x + self.width + 10, core_y + 12),
            (core_x - 10, core_y + 8)
        ]
        pygame.draw.polygon(screen, shell_color_light, cannon_points)

        # 在炮管上增加能量流動的線條
        pygame.draw.line(screen, energy_glow_magenta, (core_x, core_y), (self.x + self.width + 10, core_y), 5)
        ...
        # 遍佈在甲殼上的能量光脈 (讓載具看起來像活的)
        pygame.draw.line(screen, energy_glow_green, (self.x + 20, self.y + 40), (self.x + 80, self.y + 45), 2)
        pygame.draw.line(screen, energy_glow_green, (self.x + 100, self.y + 20), (self.x + 140, self.y + 35), 1)# 您原來的程式碼可以替換成上面的版本
        # 為了方便對比，這裡註解掉您的原始碼
        # pygame.draw.rect(screen, (25, 25, 112), (self.x + 45, self.y + 5, 15, 20)) # 原前部
        # pygame.draw.rect(screen, (173, 216, 230), (self.x + 10, self.y + 5, 35, 20)) # 原窗戶
        # pygame.draw.circle(screen, (255, 0, 0), (self.x + 15, self.y + 2), 3) # 原警燈1
        # pygame.draw.circle(screen, (0, 0, 255), (self.x + 45, self.y + 2), 3) # 原警燈2
        # pygame.draw.circle(screen, (0,0,0), (self.x + 10, self.y + 25), 5) # 原車輪1
        # pygame.draw.circle(screen, (0,0,0), (self.x + 50, self.y + 25), 5) # 原車輪2
    def get_rect(self):
